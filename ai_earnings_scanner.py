...
def generate_html_report(stocks: list, output_path: str):
    ...
    html += '#chat-btn{position:fixed;bottom:24px;right:24px;width:56px;height:56px;border-radius:50%;background:#238636;border:none;color:#fff;font-size:1.4em;cursor:pointer;box-shadow:0 4px 20px rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;transition:transform .2s,background .2s}'
    html += '#chat-btn:hover{background:#2ea043;transform:scale(1.1)}'
    html += '#chat-panel{position:fixed;bottom:90px;right:24px;width:360px;max-height:520px;background:#161b22;border:1px solid #30363d;border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,0.6);display:none;flex-direction:column;z-index:9998;overflow:hidden}'
    ...
    html += "<button id=\"chat-btn\" onclick=\"toggleChat()\" title=\"Ask about the scanner\">&#128172;</button>"
    html += "<div id=\"chat-panel\"><div id=\"chat-header\"><span>&#128172; AI Scanner Assistant</span><button id=\"chat-close\" onclick=\"toggleChat()\">&#10005;</button></div>"
    ...
    html += "<div id=\"chat-input-row\"><input id=\"chat-input\" type=\"text\" placeholder=\"Ask a question...\" onkeydown=\"if(event.key==='Enter'&&!event.shiftKey)sendMsg()\"/><button id=\"chat-send\" onclick=\"sendMsg()\">Send</button></div></div>"
    html += "<script>var chatOpen=false;function toggleChat(){chatOpen=!chatOpen;document.getElementById('chat-panel').classList.toggle('open',chatOpen);}function sendMsg(){var i=document.getElementById('chat-input');var m=i.value.trim();if(!m)return;appendMsg(m,'user');i.value='';var b=document.getElementById('chat-send');b.disabled=true;b.textContent='...';appendMsg('','bot',true);fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:m})}).then(function(r){return r.ok?r.json():Promise.reject('Server error')}).then(function(d){var t=document.querySelector('.msg.loading');if(t)t.remove();appendMsg(d.reply||'Got it.','bot')}).catch(function(e){var t=document.querySelector('.msg.loading');if(t)t.remove();appendMsg('Error: '+e.message,'bot')}).finally(function(){b.disabled=false;b.textContent='Send'})}function appendMsg(text,role,loading){var ms=document.getElementById('chat-msgs');var d=document.createElement('div');d.className='msg msg-'+role;if(loading)d.classList.add('loading');d.textContent=role==='bot'&&loading?'Thinking...':text;ms.appendChild(d);ms.scrollTop=ms.scrollHeight}</script>"
    html += "</body></html>"
