...
def generate_html_report(stocks: list, output_path: str):
    ...
    html += '<script>var LST=localStorage.getItem("sub_pop_last")||0;var NOW=Date.now();if(NOW-LST>3600000){setTimeout(function(){document.getElementById("sub-popup").classList.add("show");localStorage.setItem("sub_pop_last",NOW)},8000)}</script>'
