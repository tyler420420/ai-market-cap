...
def generate_html_report(stocks: list, output_path: str):
    ...
    for i, stock in enumerate(stocks[:20], 1):
        ...
        days_color = '#ff4444' if stock.days_to_earnings == 0 else ('#00ff88' if stock.days_to_earnings <= 7 else '#ffcc00')
    ...
    def days_color(days):
        return '#ff4444' if days == 0 else ('#00ff88' if days <= 7 else '#ffcc00')
    def news_link(news):
        if not news:
            return '--'
        url = news.get('url', '')
