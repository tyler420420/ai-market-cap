...
def generate_html_report(stocks: list, output_path: str):
    ...
    for i, stock in enumerate(stocks[:20], 1):
        ...
        days_color = '#00ff88' if stock.days_to_earnings == 0 else ('#58a6ff' if stock.days_to_earnings <= 7 else '#ffcc00')
    ...
    ticker_items += '<a href="https://invite.kraken.com/JDNW/xtswgo8o" target="_blank" style="display:inline-flex;align-items:center;gap:6px;padding:0 18px;border-right:1px solid #30363d;flex-shrink:0;text-decoration:none"><span style="font-size:1em">&#x1F4B8;</span><span style="font-weight:bold;color:#00ff88;font-size:0.9em">$300 Bonus</span><span style="color:#8b949e;font-size:0.78em">Kraken</span></a>'
    ...
    def days_color(days):
        return '#00ff88' if days == 0 else ('#58a6ff' if days <= 7 else '#ffcc00')
    def news_link(news):
        if not news:
            return '--'
        url = news.get('url', '')
