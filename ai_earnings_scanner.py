...
def generate_html_report(stocks: list, output_path: str):
    ...
    html += '<div id=sub-popup><div id=sub-popup-box><h2>Unlock Full Scanner Power</h2><p>Free users get 1 scan/day. Subscribe for 2 additional scans, real-time alerts, and the AI Trading Chat Assistant.</p><div id=sub-popup-features><div><span>&#10003;</span> 2 additional scans per day</div><div><span>&#10003;</span> Real-time earnings alerts</div><div><span>&#10003;</span> AI Trading Chat Assistant</div><div><span>&#10003;</span> Priority pre-earnings picks</div></div><a href="/pricing" id=sub-popup-btn>Subscribe Now</a><button id=sub-popup-close onclick="document.getElementById(\'sub-popup\').classList.remove(\'show\')">Maybe Later</button></div></div>'
    html += '<script>var LST=localStorage.getItem("sub_pop_last")||0;var NOW=Date.now();if(NOW-LST>3600000){setTimeout(function(){document.getElementById("sub-popup").classList.add("show");localStorage.setItem("sub_pop_last",NOW)},30000)}</script>'
    ...
    buttons_row = (
        ...
        '<span style="background:#161b22;border:1px solid #1f6feb;border-radius:5px;padding:3px 10px;font-size:0.82em"><span style="font-weight:bold;color:#58a6ff">' + str(sum(1 for s in stocks if round(s.composite_score) < 80)) + '</span> <span style="color:#8b949e">Watch</span></span>'
        '</div>'
        '</div>'
    )
