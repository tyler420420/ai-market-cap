...
# The JS renderTable builds rows - we need to add data-label to each <td>
# Current pattern: html+='<tr style="background:'+bg+'"><td>...
# New pattern: html+='<tr style="background:'+bg+'"><td data-label="Ticker">...

# Find and replace the renderTable row building
old_js = (
    "html+='<tr style=\"background:'+bg+'\"><td><strong><a href=\"https://finance.yahoo.com/quote/'+r.ticker+'\" "
    "target=\"_blank\" style=\"color:#66b2ff\">'+r.ticker+'</a></strong></td>';"
    "html+='<td>'+r.company_name.substring(0,35)+(r.company_name.length>35?'...':'')+'</td>';"
    "html+='<td><strong style=\"color:'+c+'\">'+r.score+'</strong></td>';"
...
if old_js in content:
    print('FOUND - replacing JS renderTable')
    new_js = (
        "html+='<tr style=\"background:'+bg+'\"><td data-label=\"Ticker\"><strong><a href=\"https://finance.yahoo.com/quote/'+r.ticker+'\" "
        "target=\"_blank\" style=\"color:#66b2ff\">'+r.ticker+'</a></strong></td>';"
    ...
    if idx >= 0:
        ...
        print(repr(content[idx:idx+200]) if idx >= 0 else 'not found')