...
print(f'Function at lines {start+1}-{end+1}')

new_func = '''def fetch_ai_stocks_from_finviz() -> List[str]:
    """Fetch tech sector stocks $10B+ market cap from finviz (large caps only)."""
    try:
        import requests
        from bs4 import BeautifulSoup
        tickers = set()
        for page_start in range(1, 1001, 20):
            url = f"https://finviz.com/screener.ashx?v=152&f=sec_technology,cap_large&o=ticker&r={page_start}"
        ...
        print(f"[AI Stocks] Fetched {len(result)} tech stocks ($10B+) from finviz")
        return result
...
import ast
ast.parse(open('ai_earnings_scanner.py', 'r', encoding='utf-8').read())
print('Syntax: OK')