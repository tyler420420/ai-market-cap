...
# For fmtEdate - use chr() to avoid Python parsing issues
fmtEdate_js = "function fmtEdate(s){if(!s)return'';var p=s.split('-');return parseInt(p[1])+'-'+p[2]+String.fromCharCode(60,98,114,62)+'<sub>'+p[0]+'</sub>';}"

for i, l in enumerate(lines):
    if "function scoreColor(s){return s>=80?'#00ff88':'" in l:
        lines[i] = l.replace(
            "function scoreColor(s){return s>=80?'#00ff88':'#58a6ff';}",
            "function scoreColor(s){return s>=80?'#00ff88':'#58a6ff';}" + fmtEdate_js
        )
        changes += 1
        break
