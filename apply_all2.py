...
for i, l in enumerate(lines):
    if "html += '<meta charset=\"UTF-8\"><title>' in l:
        lines[i] = l.replace(
...
# 2. btn CSS - replace the line with btn and refreshBtn styles
for i, l in enumerate(lines):
...
# 8. fmtEdate function in JS string (add after scoreColor)
for i, l in enumerate(lines):
    if "function scoreColor(s){return s>=80?'#00ff88':'" in l:
        lines[i] = l.replace(
            "function scoreColor(s){return s>=80?'#00ff88':'#58a6ff';}",
            "function scoreColor(s){return s>=80?'#00ff88':'#58a6ff';}function fmtEdate(s){if(!s)return'';var p=s.split('-');return parseInt(p[1])+'-'+p[2]+'<br><sub>'+p[0]+'</sub>';}"
        )
        changes += 1
        break

# 9. earnings_date cell in renderTable
for i, l in enumerate(lines):
...
# Check syntax
try:
    compile(open('ai_earnings_scanner.py','r',encoding='utf-8').read(), 'ai_earnings_scanner.py', 'exec')
    print('Syntax OK')
